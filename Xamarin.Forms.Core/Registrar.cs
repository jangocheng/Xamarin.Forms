...
namespace Xamarin.Forms.Internals
{
	...
	public class Registrar<TRegistrable> where TRegistrable : class
	{
		...
		static Type _defaultVisualRenderer = typeof(Visual.DefaultVisual);
		static Type[] _defaultVisualRenderers = new[] { _defaultVisualRenderer };
		...
		internal TRegistrable GetHandler(Type type) => GetHandler(type, _defaultVisualRenderer);

		internal TRegistrable GetHandler(Type type, Type visualType)
		{
			Type handlerType = GetHandlerType(type, visualType ?? _defaultVisualRenderer);
			if (handlerType == null)
				return null;
		...
		public TOut GetHandlerForObject<TOut>(object obj, IVisual visual, params object[] args) where TOut : class, TRegistrable
		{
			if (obj == null)
				throw new ArgumentNullException(nameof(obj));
		...
		public Type GetHandlerType(Type viewType) => GetHandlerType(viewType, _defaultVisualRenderer);

		public Type GetHandlerType(Type viewType, Type visualType)
		{
			visualType = visualType ?? _defaultVisualRenderer;
